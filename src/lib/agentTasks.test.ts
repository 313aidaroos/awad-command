  ...
  const store: AgentTaskStore = {
    from(table) {
      ...
      return {
        ...
        async insert(row) {
          if (opts.failInsert === table) return { data: null, error: { message: `${table} blocked` } };
...
describe('persistApprovalResolution', () => {
  it('updates both approval and task rows on approve', async () => {
    ...
    const resolved = await persistApprovalResolution(created.approvalId!, 'approved', { supabase: store });
  ...
  it('cancels the task on deny', async () => {
    ...
    await persistApprovalResolution(created.approvalId!, 'denied', { supabase: store });
  ...
  it('keeps a demo no-op when the service client is missing', async () => {
    const resolved = await persistApprovalResolution('apr_1', 'approved', { supabase: null });
    expect(resolved).toMatchObject({ ok: true, demo: true, decision: 'approved' });
  });
});
