...
export const UNIVERSE_ZOOM = 32;
export const INTERIOR_ZOOM = 11.5;
export const CHAMBER_RADIUS = 13.8;

export const UNIVERSE_CAM: CameraTarget = {
  position: [0, 6.6, UNIVERSE_ZOOM],
  lookAt: [0, 0.25, 0],
  duration: 1.8,
  phase: 'universe',
};
...
/** Approach the entity, pause at the threshold, then sit inside the hangar — never inside a 2-unit bubble. */
export function projectEnterSequence(pos: [number, number, number]): CameraTarget[] {
  ...
  return [
    ...
    {
      position: [x + sx * 7.2 - dx * 1.1, y + 2.55, z + sz * 7.2 - dz * 1.1],
      lookAt: [x, y - 0.45, z],
      duration: 1.45,
      phase: 'interior',
    },
