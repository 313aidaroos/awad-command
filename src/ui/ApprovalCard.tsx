...
import { resolveApprovalAction } from '@/app/approvals/actions';
...
export function ApprovalCard() {
  ...
  const canApprove = (!needsConfirm || confirm.toLowerCase() === 'confirm') && !busy;
  const recordOnly = !item.taskId;

  async function decide(decision: 'approved' | 'denied') {
    setBusy(true);
    setError(null);
    const result = await resolveApprovalAction(item.id, decision);
    if (!result.ok) {
      setError(result.error ?? 'Could not update approval');
      setBusy(false);
      return;
    }
    resolve(item.id, decision);
    setBusy(false);
    close();
  }
  ...
  return (
    <Glass className="fixed right-4 bottom-28 z-40 w-[320px] p-4">
      ...
      {error ? <p className="mt-2 text-[11px] text-[var(--s-attention)]">{error}</p> : null}
