...
import {
  composeLeadOutboundMessage,
  parseLeadDirective,
} from "@/ceo/leadDirective";
import {
  CEO_ANTHROPIC_TOOLS,
  CEO_SYSTEM_PROMPT,
  composeCeoText,
...
export async function runDemoCeoTurn(
  input: CeoTurnInput,
  deps: CeoTurnDeps = {},
...
export async function runAnthropicCeoTurn(
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
    ...
    text = completion.content
      .filter(
        (block): block is Extract<AnthropicContent, { type: "text" }> =>
          block.type === "text",
      )
    ...
    for (const tool of toolUses) {
      const input =
        tool.input && typeof tool.input === "object" ? tool.input : {};
